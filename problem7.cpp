/*
Name of Program: problem7.cpp
Author: Logan Whitt
KUID: 3152587

Description: takes in row index, column index, and new value
and replaces the value at that point
Outputs: the given matricies and the outputted new matrix
...
Matrix update_element_of_matrix(Matrix matrix, int element = 100,int row_index = 0, int column_index = 0){
    if (row_index < 0 || row_index >= matrix.size_of_matrix){ //if the given original row is out of range
        cout << "Invalid row index given." << endl;
        return matrix;
    }
    if (column_index < 0 || column_index >= matrix.size_of_matrix){ //if the given ending row is out of range
        cout << "Invalid column index given." << endl;
        return matrix;
    }
    
    matrix.rows[row_index][column_index] = element;    //update element at that index
    return matrix;  //return new matrix
}
...
int main(){
    string filepath = "test.txt"; //testing filepath
    ...
    Matrix swapped_matrix_one = update_element_of_matrix(matricies[0]);
